...
public class ConferencePlatformApplication {
    ...
    public CommandLineRunner initDatabase(ConferenceRoomRepository conferenceRoomRepository, ConferenceRepository conferenceRepository) {
        return args -> {
            ...
            ConferenceRoom conferenceRoom1 = ConferenceRoom.builder().name("room one").status(RoomStatus.FREE).location("calle 1").maxCapacity(1).build();
            ConferenceRoom conferenceRoom2 = ConferenceRoom.builder().name("room two").status(RoomStatus.UNDER_CONSTRUCTION).location("calle 2").maxCapacity(3).build();
            ConferenceRoom conferenceRoom3 = ConferenceRoom.builder().name("room three").status(RoomStatus.FREE).location("calle 3").maxCapacity(6).build();
            ConferenceRoom conferenceRoom4 = ConferenceRoom.builder().name("room four").status(RoomStatus.OCCUPIED).location("calle 4").maxCapacity(9).build();
            ...
            Conference conference1 = Conference.builder().name("room one").startConference(localDateTime.minusHours(4)).endConference(localDateTime.minusHours(3)).conferenceRoom(conferenceRoom1).build();
            Conference conference2 = Conference.builder().name("room two").startConference(localDateTime.plusHours(3)).endConference(localDateTime.plusHours(4)).conferenceRoom(conferenceRoom2).build();
            Conference conference3 = Conference.builder().name("room three").startConference(localDateTime.plusHours(5)).endConference(localDateTime.plusHours(6)).conferenceRoom(conferenceRoom3).build();
            Conference conference4 = Conference.builder().name("room four").startConference(localDateTime.plusHours(1)).endConference(localDateTime.plusMinutes(30)).conferenceRoom(conferenceRoom4).build();
